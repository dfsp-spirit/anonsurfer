...
echo "$APPTAG INFO: --- Defacing subject '${SUBJECT_ID}' in directory '${SD}'. ---"
...
for REL_VOL_FILE in $VOLUME_FILES_RELATIVE_TO_MRI_DIR; do
    ...
    if [ $? -ne 0 ]; then
        echo "$APPTAG ERROR: mri_deface command failed for subject '${SUBJECT_ID}' file '${VOL_FILE}'. Exiting."
        exit 1
    else
        if [ -f "${DEFACED_FILE}" ]; then
            ...
            echo "$APPTAG ERROR: Cannot read subject '${SUBJECT_ID}' defaced filed '${DEFACED_FILE}' after mri_deface command (even though it returned no error). Exiting."
            exit 1
        fi
    fi
done
